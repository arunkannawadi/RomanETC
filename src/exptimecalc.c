 ...
 * -DREAD5 = sets read noise at 5e and dark current at 0.05 e/s/pix (instead of
 * taking user input) -DWLCUT_DEFAULT = sets default WL cuts (Res > 0.4 and
 * sigma_e < 0.2) -DOUT_WL_CAT = dumps weak lensing source catalog to a file
 * -DLOGICAL_READ_FLOOR = changes the behavior of the read noise floor from
 * quadrature to logical -DWFE_OVERRIDE = overrides control on WFE in WL mode
 * (useful to extract detection sensitivity estimates from the imager blueward
 * of 14*RMSWFE) -DWFE_HOPKINS_EXP = put wavefront error in exponential Hopkins
 * distribution -DNO_OBJ_CONTINUUM = ignores source continuum Poisson
 * fluctuations as a noise source -DUSE_SNRE = use the expected rather than
 * observed SNR for the BAO cuts -DOUT_EXTRA_PSF_PROPERTIES = write additional
 * properties of the PSF (for imaging modes) -DOIII_GAL = predict [O III]
 * properties instead of H alpha -DOII_GAL = predict [O II] properties instead
 * of H alpha
 */
...
/* This is useful to avoid repeated blocks of code -- defines BAO_OR_SPCONT_MODE
 * if we are in either the BAO or SPCONT modes.
 */
#ifdef BAO_MODE
...
/* --- CONSTANTS ASSOCIATED WITH OUTPUT THAT WE DO NOT REPEAT FOR EACH USER
 * INPUT --- */
...
double VAR_READ_FRAME; /* Read noise variance per frame (e-2) -- roughly
                          cdsnoise^2/2   */
...
/* Smearing of galaxy size due to spectral dispersion -- sigma on 1 axis in
 * arcsec */
...
#else
/* Reference equivalent width in arcsec -- 100A EW rest frame @ D_theta = 240
 * arcsec. */
...
typedef struct
{
    ...
    double dll;       /* fractional width of band, i.e. ln(lambda) = central +/-
                       * 0.5*dll;       only used if broadband */
...
typedef struct
{
    ...
    double pmsm_emissivity;    /* Emissivity of primary & secondary mirror surfaces
                                  (epsilon) */
    double outer_ann_ratio;    /* Ratio of outer diameter of beam to that of primary
                                  (rho) */
    int ismask;                /* Cold pupil mask? 1 = yes, 0 = no */
    double T_aft;              /* Temperature of aft optics [K] */
    double aft_net_emissivity; /* Net emissivity of aft optics (total of all
                                  surfaces in 'series') */
    double post_filter_rate;   /* Count rate in e-/pix/s from structures downstream
                                  of the filter */
} THERMAL_DATA;
...
typedef struct
{
    ...
    double slope1, slope2; /* d(RMS WFE)/d(lambda) [dimensionless] at blue
                            * (slope1) or red (slope2) wavelengths
...
getJ1 (double x)
{
    ...
    f = 0.;
    for (i = 0; i < 50; i++)
        f += cos (x * sin ((i + 0.5) * M_PI / 50.) - ((i + 0.5) * M_PI / 50.));
    return (0.02 * s * f);
}
...
geterf (double x)
{
    ...
    /* For values greater than 6, the result for erf will be unity to machine
     * precision */
    if (x > 6)
        return (s);
...
ConfigDetector (int det_type)
{
    ...
    switch (det_type)
        ...
        case 3: /* H4RG - 10 um, 64 channel readout */
            /* Note -- equivalent to assuming unchanged read noise for 32 channel
             * readout @ 200 kHz */
            PIXSIZE_PHYS = 10.0;
            PIX_CD = 2.94;  /* Assumed unchanged from H2RG until better data available */
        ...
        case 5: /* a 'really good' H1RG (for the IFU) - 18 um, 16 channel readout, 2x2
                   binned in post-processing */
            PIXSIZE_PHYS = 36.0;
            PIX_CD = 2.94;
            T_FRAME = 0.66;
 ...
 */
double
get_throughput (double lambda, THROUGHPUT_DATA *T)
{
    int i, N;
     ...
     */
    if (lambda <= T->lambda[0])
        return (T->throughput[0]);
    if (lambda >= T->lambda[N - 1])
        return (T->throughput[N - 1]);

    /* Interpolation */
    i = 0;
    while (lambda > T->lambda[i + 1] && i < N - 2)
        i++;
    return (T->throughput[i]
            + (T->throughput[i + 1] - T->throughput[i]) * (lambda - T->lambda[i])
                      / (T->lambda[i + 1] - T->lambda[i]));
}
 ...
 */
double
get_rmswfe_from_fcn (double lambda, WAVEFRONT_ERROR_FUNCTION *wavefront)
{
    double x;
 ...
 */
double
get_MTF_mono (double u, double v, PSF_DATA *psf)
{
    ...
    /* Aberration contribution comes from the default, which is worst case of
     * 2nd/3rd order aberrations; or the exponential Hopkins distribution, a
     * Gaussian with assumed correlation length.
     */
    T_wfe2 = 1.; /* this line is to prevent an unused variable warning; it has no
                    effect */
#ifdef WFE_HOPKINS_EXP
    T_wfe = exp (-4. * M_PI * M_PI * psf->rmswfe * psf->rmswfe * (1. - exp (-18. * k1 * k1)));
#else
    T_wfe = exp (-72. * psf->rmswfe * psf->rmswfe
                 * (1. - 0.5 * exp (-32. * k1 * k1) - 0.5 * exp (-8. * k1 * k1)));
    T_wfe2 = 0.5 * exp (-720. * psf->rmswfe * psf->rmswfe * k1 * k1)
             + 0.5 * exp (-2160. * psf->rmswfe * psf->rmswfe * k1 * k1);
    if (psf->rmswfe > 0.175)
        {
            T_wfe = T_wfe2;
        }
     ...
     */
    T_sig *= 1. - NIIFRAC + NIIFRAC * 0.247 * cos (2. * M_PI * u * 0.538)
             + NIIFRAC * 0.753 * cos (2. * M_PI * u * 0.756);
#endif

    /* The three effects are convolved in real space so multiplied in Fourier
     * space */
 ...
 */
double
get_MTF (double u, double v, PSF_DATA *psf)
{
    ...
    if (psf->is_broadband == 0)
        return (get_MTF_mono (u, v, psf));
 ...
 * effective radius (assumed exponential) and given PSF.  The result is given
 * by:
 *
 * = r * int dk <G~(k) f~(k)> J_1(kr).
 *
 * where f~(k) is the FT of the galaxy profile, and <> is an angular average.
 * In its present version, the galaxy is assumed circular and the angular
 * average is done with angular steps of 10 degrees.
 *
 * The function is well behaved if reff=0.
 */
double
get_gal_fracE (double r, double reff, PSF_DATA *psf)
{
 ...
 */
double
get_gal_size (double eFrac, double reff, PSF_DATA *psf)
{
    double r, ratio;

    r = 0.2;
    ratio = 0.5;
    while (get_gal_fracE (r, reff, psf) > eFrac && r < 206265)
        r /= exp (ratio);
    while (get_gal_fracE (r, reff, psf) < eFrac && r > 1e-6)
        r *= exp (ratio);
 ...
 */
double
get_gal_Omeff (double reff, PSF_DATA *psf)
{
    double k, k2pi, kmax, dk, I, rs, dphi;
    long j, ndk, jphi, nphi;
 ...
 */
double
get_gal_PeakInt (double reff, PSF_DATA *psf)
{
    double k, k2pi, kmax, dk, I, rs, dphi;
    long j, ndk, jphi, nphi;
    double u, v, tildef, prod;
 ...
 */
double
get_gal_thetaeff (double reff, PSF_DATA *psf)
{
 ...
 */
double
get_J_couplingIntegral (double reff, PSF_DATA *psf)
{
 ...
 * purposes, i.e.
 *
 * c_int = int [G~(u) f~(u) 2 pi u_x]^2 d^2u = (2pi)^-2 int [G~(k) f~(k) k_x]^2
 * d^2k,
 *
 * where G~ = MTF and f~ = galaxy Fourier transform.
 * This is defined so that if one had a perfect template for the location and
 ...
 *
 * ivar(centroid) = c_int * [galaxy counts]^2/[background counts per solid
 * angle]
 *
 * This is just the statistical centroid error and does not account for aliasing
 * -- so should be used with caution for very small sources or if it gives
 * errors of a tiny fraction of a pixel.
 */
double
get_gal_centroid_integral (double reff, PSF_DATA *psf)
{
    double k, k2pi, kmax, dk, I, rs, dphi;
    long j, ndk, jphi, nphi;
    double u, v, tildef, prod;
 ...
 * routine does the full integral for an exponential profile galaxy and the true
 * PSF.
 *
 ...
 * A_{eff,s}^{-1} = int [G~(u) d/dlnu f~(u)]^2 d^2u = (2pi)^-2 int [G~(k) d/dlnk
 * f~(k)]^2 d^2k,
 *
 * where G~ = MTF and f~ = galaxy Fourier transform.
 ...
 */
double
get_shape_penalty_factor (double reff, PSF_DATA *psf)
{
    double k, k2pi, kmax, dk, I, rs, dphi;
    long j, ndk, jphi, nphi;
...
/* Gets the 1 sigma flux uncertainty for a line in a single exposure for a
 * galaxy with: effective radius reff (arcsec) specified psf variance per unit
 * area (e-^2/arcsec^2) calib_1exp (e- per (W/m2))
 *
 * Output in W/m2.
 */
double
get_1sigma_flux_1exp (double reff, PSF_DATA *psf, double var_1exp, double calib_1exp)
{
    double Omeff;
...
/* Gets the Z sigma flux uncertainty for a line in a single exposure for a
 * galaxy with: effective radius reff (arcsec) specified psf variance per unit
 * area (e-^2/arcsec^2) calib_1exp (e- per (W/m2)) equivalent width*dispersion
 * EWD (arcsec)
 *
 * Output in W/m2.
 */
double
get_limflux (double reff, PSF_DATA *psf, double var_1exp, double calib_1exp, double EWD, double Nexp,
             double Z)
{
     ...
     * F = Z * sqrt((var_1exp + fd*J/EWD*F*calib_1exp)*Omeff)/calib_1exp /
     * sqrt(Nexp)
     *
     * The solution to this equation is to first find the coefficients by
     * squaring: F^2 = alpha + 2*beta*F and then F = beta + sqrt(beta^2 + alpha)
     */
 ...
 * given r_eff, number of exposures, noise (var_1exp), and calibration
 * (calib_1exp), and the specified maximum ellipticity error.
 *
 * If WL_SNR_MIN is defined, we also require a detection at this number of
 * sigmas.
 */
double
get_maglim_shape (double r_eff, PSF_DATA *psf, int N_exp, double var_1exp, double calib_1exp,
                  double max_ellip_err)
{
...
/* Gets limiting AB magnitude for successful shape measurement on a source of
 * given r_eff, number of exposures, noise (var_1exp), and calibration
 * (calib_1exp), and the specified maximum ellipticity error. NO S/N cut is
 * imposed in this case; it is useful for estimating the ellipticity error by
 * comparing the magnitude of a particular galaxy to the "limiting" magnitude
 * (with known sigma_e).
 */
double
get_maglim_shape_noSNRcut (double r_eff, PSF_DATA *psf, int N_exp, double var_1exp, double calib_1exp,
                           double max_ellip_err)
{
 ...
 */
void
get_galsizes (double z, double FHa, double *med_reff, double *dispersion, int model)
{
     ...
     * dust reddening of 0.27 mag between the two lines following Khostovan et al.
     * (2015)
     */
    if (model % 10 == 2)
        {
     ...
     * Lognormal reff distributions fit in 5 bins centered at log10 FHa(cgs) =
     * -16.0(0.2)-15.2 Can be approximated to ~0.01dex (except for last bin) by:
 ...
 *
 * The [NII]/Ha ratio assumed by each model is given by get_HaLF2 to the pointer
 * NII_Ha.
 *
 ...
 */
double
get_HaLF (double z, double FHa, int model)
{
    double get_HaLF2 (double, double, int, double *);
    return (get_HaLF2 (z, FHa, model, NULL));
}
double
get_HaLF2 (double z, double FHa, int model, double *NII_Ha)
{
    ...
    /* Average of H alpha */
    if (model / 10 == 99)
        return ((get_HaLF2 (z, FHa, 60 + model % 10, NII_Ha) + get_HaLF2 (z, FHa, 70 + model % 10, NII_Ha)
                 + get_HaLF2 (z, FHa, 80 + model % 10, NII_Ha))
                / 3.);
    /* Average of [OIII] */
    if (model / 10 == 199)
        return ((get_HaLF2 (z, FHa, 1000 + model % 10, NII_Ha) + get_HaLF2 (z, FHa, 1010 + model % 10, NII_Ha)
                 + get_HaLF2 (z, FHa, 1020 + model % 10, NII_Ha))
                / 3.);
    ...
    if (NII_Ha != NULL)
        *NII_Ha = 0.; /* turn off the "[N II]" feature since [O III] is not blended
                         with another line */
     ...
     * Monthly Notices of the Royal Astronomical Society, Volume 402, Issue 2, pp.
     * 1330-1338. Schechter function with phi* = 1.35e-3 Mpc^-3, alpha=-1.35,
     * L*=5.1e34(1+z)^3.1 W (except above z=1.3 fix at L*=6.8e35 W)
     */
    if (model / 10 == 0)
        {
    ...
    /* Sobral et al 2012 with cosmology corrections (to WMAP5) and aperture
     * corrections assuming exponential profile with median r_eff at 1e-19 W/m^2.
     */
    if (model / 10 == 3)
        {
            Lstar = z < 0.84   ? 34.70176145 + 0.500472054 * (z - 0.40)
                    : z < 1.47 ? 34.92196916 + 0.495788251 * (z - 0.84)
                    : z < 2.23 ? 35.23431576 + 0.394312857 * (z - 1.47)
                               : 35.53399353;
            Lstar = pow (10, Lstar);
            phistar = z < 0.84   ? -3.107120757 + 1.434831821 * (z - 0.40)
                      : z < 1.47 ? -2.475794755 - 0.247531478 * (z - 0.84)
                      : z < 2.23 ? -2.631739587 - 0.170782235 * (z - 1.47)
                                 : -2.761534085;
    ...
    /* Colbert et al 2013 fit -- as presented in the paper, and corrected to WMAP5
     * cosmology. Parameters are: z=0.6: logPhistar = -2.51+0.00, logLstar
     * = 34.72-0.01, alpha = -1.27 z=1.2: logPhistar = -2.70-0.01, logLstar
     * = 35.18+0.00, alpha = -1.43
     */
    if (model / 10 == 5)
        {
...
void
print_HaLF_model (FILE *fp, int model)
{

#ifdef OIII_GAL
    if (model / 10 == 100)
        {
            fprintf (fp, "Mehta et al. (2015) [O III]");
            return;
        }
    if (model / 10 == 101)
        {
            fprintf (fp, "Colbert et al. (2013) [O III]");
            return;
        }
    if (model / 10 == 102)
        {
            fprintf (fp, "Khostovan et al. (2015) H beta + [O III]");
            return;
        }
    if (model / 10 == 199)
        {
            fprintf (fp, "[O III] 3-model ave.");
            return;
        }
#elif OII_GAL
    if (model / 10 == 200)
        {
            fprintf (fp, "Khostovan et al. (2015) [O II]");
            return;
        }
#else
    if (model / 10 == 0)
        {
            fprintf (fp, "Geach et al (2009), 50%c confidence", '%');
            return;
        }
    if (model / 10 == 1)
        {
            fprintf (fp, "Geach et al (2009), 90%c confidence", '%');
            return;
        }
    if (model / 10 == 2)
        {
            fprintf (fp, "Geach et al (2009), reduced /1.257");
            return;
        }
    if (model / 10 == 3)
        {
            fprintf (fp, "Sobral et al (2012), corrected, best fit");
            return;
        }
    if (model / 10 == 4)
        {
            fprintf (fp, "Colbert et al (2013), prepublication, best fit");
            return;
        }
    if (model / 10 == 5)
        {
            fprintf (fp, "Colbert et al (2013), best fit");
            return;
        }
    if (model / 10 == 6)
        {
            fprintf (fp, "Global fit, HIZELS+WISP+NICMOS - Model 3");
            return;
        }
    if (model / 10 == 7)
        {
            fprintf (fp, "Global fit, Euclid - Model 1 (LDE)");
            return;
        }
    if (model / 10 == 8)
        {
            fprintf (fp, "Global fit, Euclid - Model 2 (LE)");
            return;
        }
    if (model / 10 == 99)
        {
            fprintf (fp, "H alpha 3-model ave.");
            return;
        }
#endif
 ...
 * Takes as input the ratio [NII]/Ha (where [NII] is the *total* of the [NII]
 * lines). Based on methodology from the effective area determination.
 */
double
get_SNR_boost_factor (double z, double reff, double NII_Ha, PSF_DATA *psf)
{
    ...
    double I1, I2; /* The integral I1 will represent the total SNR^2 integral,
                    * whereas I2 will include only the H alpha. The SNR boost is
                    * the sqrt of the ratio.
                    */
                    ...
                    if (fabs (u) < 0.5 / psf->pixscale && fabs (v) < 0.5 / psf->pixscale)
                        {
                            ...
                            /* Note only u appeared in the squared transfer function for the
                             * 'triplet' since the lines are separated only in the x-direction.
                             */
...
/* Probability of a given galaxy being above the detection threshold at
 * specified redshift and H alpha flux. This does not include photometric
 * scatter.
 *
 * Input units: FHa (W/m2), var_1exp(e-^2/pix), calib_1exp(e- per W/m^2)
 */
double
get_P_Detectable (double z, double FHa, PSF_DATA *psf, double var_1exp, double calib_1exp, int N_exp,
                  double significance_cut, int model)
{
    ...
    /* Need to determine the critical number of sigmas of the lognormal
     * distribution in size at which the galaxies are detected. Use x =
     * ln(reff/med_reff)/dispersion. If the critical threshold is outside 8 sigma,
     * this bisection routine will fail. However if this happens then the galaxies
     * are essentially all detected or not anyway.
     */
    x = 0;
    for (dx = 4; dx > 1e-6; dx /= 2.)
        {
            ...
            /* This shouldn't happen. This really shouldn't be <1, but the limit is set
             * at 0.98 for numerical reasons.
             */
            if (boost < 0.98)
                {
                    fprintf (stderr, "Error: boost=%8.6lf\n", boost);
                    fprintf (stderr, "Diagnostics: z=%6.4lf reff=%7.4lf NII_Ha=%7.4lf disp=%7.2lf as/um\n", z,
                             reff, NII_Ha, psf->linDisp);
                    exit (1);
                }
#endif
    ...
    P = 0.5 * (1. + geterf (x));
    if (x <= -4.0)
        P = 0; /* If we can only see objects with -4 sigma size fluctuations,
                * let's just call them nondetections.
                */
    return (P);
}

/* Obtain the comoving density of detectable galaxies per ln Flux, at a given
 * redshift and flux level and ancillary parameters.
 *
 * Output units are Mpc^-3.
 * Input units: FHa (W/m2), var_1exp(e-^2/pix), calib_1exp(e- per W/m^2)
 *
 * Here "detectable" means above threshold -- does not count losses due to
 * bright stars etc.
 *
 * If USE_SNRE is set, then we simply take the probability of SNRe being greater
 * than the threshold. Otherwise (DEFAULT) we want the probability of SNRo being
 * greater than the threshold -- this means a 2-point integration over the
 * distribution of SNRe.
 */
double
get_n_Detectable (double z, double FHa, PSF_DATA *psf, double var_1exp, double calib_1exp, int N_exp,
                  double significance_cut, int model)
{

#ifdef USE_SNRE
    return (get_P_Detectable (z, FHa, psf, var_1exp, calib_1exp, N_exp, significance_cut, model)
            * get_HaLF (z, FHa, model));
#else
    /* DEFAULT */
    double P1, P2;
    P1 = get_P_Detectable (z, FHa, psf, var_1exp, calib_1exp, N_exp,
                           significance_cut + 1. - 0.5 / significance_cut, model);
    P2 = get_P_Detectable (z, FHa, psf, var_1exp, calib_1exp, N_exp,
                           significance_cut - 1. - 0.5 / significance_cut, model);
    return (0.5 * (P1 + P2) * get_HaLF (z, FHa, model));
#endif
}

/* Obtain the total comoving density of detectable galaxies, and (if requested)
 * the statistics of their flux distribution:
 *
 ...
 */
double
get_n_galaxies (double z, PSF_DATA *psf, double var_1exp, double calib_1exp, int N_exp,
                double significance_cut, int model, double *stats)
{
    ...
    /* Get minimum detectable flux for a point source in W/m2, divided by 2 to
     * account for objects that up-scatter due to statistical fluctuations.
     */
    Fmin = get_1sigma_flux_1exp (0, psf, var_1exp, calib_1exp) * significance_cut / sqrt ((double)N_exp) / 2.;

    /* Compute comoving density per flux bin up to 4 orders of magnitude brighter
     * than min flux. Track r = ln(F/Fmin) and compute the integral over all
     * galaxies of r^j (j=0..4).
     */
    dlnF = 0.02 * log (10);
    for (k = 0; k < NBIN; k++)
        {
            r = (k + 0.5) * dlnF;
            thisn[k] = get_n_Detectable (z, Fmin * exp (r), psf, var_1exp, calib_1exp, N_exp,
                                         significance_cut, model)
                       * dlnF;
        }
 ...
 * effective radius and redshift ranges. These are [reffmin,reffmax) and
 * [zmin,zmax) so as to ensure that overlapping ranges are added together
 * consistently.
 *
 * If dNeffdA!=NULL assigns the value of effective number density (measured
 * relative to intrinsic ellipticity dispersion 0.4 rms per axis).
 *
 ...
 */
double
get_dNdA_WL_gal2 (double reffmin, double reffmax, double zmin, double zmax, char GalaxyCat[],
                  ...
                  double calib_1exp, double max_ellip_err, double *dNeffdA, char OutFile[], char outmode)
{
    ...
    if (reffmin >= reffmax || zmin >= zmax)
        return (0);

    /* Generate limiting magnitudes in each range */
    for (i = 0; i < NRBINS; i++)
        {
            maglim[i] = get_maglim_shape (reffmin * pow (reffmax / reffmin, (i + 0.5) / (double)NRBINS), psf,
                                          N_exp, var_1exp, calib_1exp, max_ellip_err);
            maglim_noSNRcut[i] = get_maglim_shape_noSNRcut (
                    reffmin * pow (reffmax / reffmin, (i + 0.5) / (double)NRBINS), psf, N_exp, var_1exp,
                    calib_1exp, max_ellip_err);
        }
    ...
    /* Basic data: number of wavelength points, number of galaxies, area of
     * catalog in deg^2 */
  ...
  exit(1);
    ...
    /* Open output file */
    fq = NULL; /* This line does nothing except suppress a spurious compiler
                  warning */
    if (OutFile != NULL)
        {
                    ...
                    for (i = 0; i < 10; i++)
                        {
                            ...
                            iw = 0;
                            while (iw < Nw - 2 && cw[iw + 1] < lambda)
                                iw++;
                            frac = log (lambda / cw[iw]) / log (cw[iw + 1] / cw[iw]);
                            galflux += exp (-(abmag[iw] * (1 - frac) + abmag[iw + 1] * frac)
                                            / 1.085736204758129);
                        }
                    galflux /= 10;
                            ...
                            if (OutFile != NULL)
                                {
                                    ...
                                    fprintf (fq, "%7ld %6.4lf %6.4lf %7.5lf %6.4lf %11.5lE\n", id, z, r_eff,
                                             sig_e, galmag, calib_1exp * pow (10, -.4 * galmag));
#else
...
double
get_dNdA_WL_gal (double reffmin, double reffmax, double zmin, double zmax, char GalaxyCat[],
                 double lambda_min, double lambda_max, PSF_DATA *psf, int N_exp, double var_1exp,
                 double calib_1exp, double max_ellip_err, double *dNeffdA)
{

    return (get_dNdA_WL_gal2 (reffmin, reffmax, zmin, zmax, GalaxyCat, lambda_min, lambda_max, psf, N_exp,
 ...
 */
double
Galactic_Alambda__EBV (double lambda)
{
    ...
    if (lambda < .1 || lambda > 10)
        {
            fprintf (stderr,
                     "Error: Wavelength lambda = %12.5lE out of range for dust "
                     "extinction law.\n",
                     lambda);
            wait_for_user ();
            exit (1);
 ...
 *
 * Caveats:
 *   No allowance is made for annual variations (due to tilt of Ecliptic
 * relative to dust midplane) Range of valid wavelengths = 0.22--2.50 microns
 * (in particlar: neglected thermal emission)
 */
double
get_zodi_bkgnd (double ecl_lat, double ecl_dlon, double lambda_min, double lambda_max, THROUGHPUT_DATA *T)
{
    ...
    /* Sky brightness table: rows are varying ecliptic latitude, cols are varying
     * longitude, at the values shown.
     *
     * This is at 0.5um in units of 1e-8 W/m^2/sr/um.
     * Electronic version of Table 17 of Leinert (1997), except for placeholders
     * (1's) at elongation <15 degrees (where we should not use this routine
     * anyway!).
     */
    int nlat = 11;
    int nlon = 19;
    ...
    double skyTable[]
            = { 1,    1,    1,    3140,  1610, 985,  640,  275,  150,  100,  77,   1,    1,    1,    2940,
                1540, 945,  625,  271,   150,  100,  77,   1,    1,    4740, 2470, 1370, 865,  590,  264,
                148,  100,  77,   11500, 6780, 3440, 1860, 1110, 755,  525,  251,  146,  100,  77,   6400,
                4480, 2410, 1410, 910,   635,  454,  237,  141,  99,   77,   3840, 2830, 1730, 1100, 749,
                545,  410,  223,  136,   97,   77,   2480, 1870, 1220, 845,  615,  467,  365,  207,  131,
                95,   77,   1650, 1270,  910,  680,  510,  397,  320,  193,  125,  93,   77,   1180, 940,
                700,  530,  416,  338,   282,  179,  120,  92,   77,   910,  730,  555,  442,  356,  292,
                250,  166,  116,  90,    77,   505,  442,  352,  292,  243,  209,  183,  134,  104,  86,
                77,   338,  317,  269,   227,  196,  172,  151,  116,  93,   82,   77,   259,  251,  225,
                193,  166,  147,  132,   104,  86,   79,   77,   212,  210,  197,  170,  150,  133,  119,
                96,   82,   77,   77,    188,  186,  177,  154,  138,  125,  113,  90,   77,   74,   77,
                179,  178,  166,  147,   134,  122,  110,  90,   77,   73,   77,   179,  178,  165,  148,
                137,  127,  116,  96,    79,   72,   77,   196,  192,  179,  165,  151,  141,  131,  104,
                82,   72,   77,   230,   212,  195,  178,  163,  148,  134,  105,  83,   72,   77 };

    /* Solar spectrum: in units of W/m^2/sr/um at log10(lambda/um) =
     * -0.80(0.01)+0.40 Ref: Colina, Bohlin, Castelli 1996 AJ 112, 307
     *
     * V band (550 nm) is SolarSpec[54]
     */
    double SolarSpec[]
            = { 1.87138e-01, 2.61360e-01, 4.08020e-01, 6.22197e-01, 9.02552e-01, 1.51036e+00, 2.25890e+00,
                2.75901e+00, 4.03384e+00, 5.42817e+00, 7.26182e+00, 1.01910e+01, 2.01114e+01, 3.62121e+01,
                4.31893e+01, 5.43904e+01, 4.91581e+01, 4.95091e+01, 4.95980e+01, 5.93722e+01, 5.27380e+01,
                1.02502e+02, 1.62682e+02, 2.53618e+02, 2.01084e+02, 2.08273e+02, 4.05163e+02, 5.39830e+02,
                5.31917e+02, 6.31200e+02, 7.06134e+02, 8.13653e+02, 1.00508e+03, 9.56536e+02, 9.50568e+02,
                9.82400e+02, 1.06093e+03, 1.12669e+03, 1.09922e+03, 1.10224e+03, 1.36831e+03, 1.72189e+03,
                1.74884e+03, 1.59871e+03, 1.74414e+03, 1.98823e+03, 2.02743e+03, 2.00367e+03, 2.03584e+03,
                1.90296e+03, 1.93097e+03, 1.86594e+03, 1.86655e+03, 1.87957e+03, 1.87978e+03, 1.83915e+03,
                1.84447e+03, 1.80371e+03, 1.76779e+03, 1.70796e+03, 1.66589e+03, 1.61456e+03, 1.53581e+03,
                1.51269e+03, 1.44957e+03, 1.39215e+03, 1.34031e+03, 1.28981e+03, 1.24501e+03, 1.19548e+03,
                1.15483e+03, 1.10546e+03, 1.06171e+03, 9.94579e+02, 9.54006e+02, 9.15287e+02, 8.63891e+02,
                8.31183e+02, 7.95761e+02, 7.62568e+02, 7.27589e+02, 6.94643e+02, 6.60883e+02, 6.21830e+02,
                5.83846e+02, 5.59624e+02, 5.34124e+02, 5.06171e+02, 4.80985e+02, 4.63139e+02, 4.39482e+02,
                4.13122e+02, 3.94543e+02, 3.75591e+02, 3.56069e+02, 3.35294e+02, 3.16374e+02, 2.98712e+02,
                2.82737e+02, 2.69581e+02, 2.49433e+02, 2.36936e+02, 2.21403e+02, 2.04770e+02, 1.87379e+02,
                1.75880e+02, 1.60408e+02, 1.46210e+02, 1.36438e+02, 1.24412e+02, 1.16500e+02, 1.07324e+02,
                9.89669e+01, 9.12134e+01, 8.28880e+01, 7.71064e+01, 7.06245e+01, 6.42367e+01, 5.87697e+01,
                5.39387e+01, 4.98208e+01 };
    ...
    if (elon < 15)
        {
            fprintf (stderr,
                     "Error: get_zodi_bkgnd: elongation = %12.5lE degrees out of valid "
                     "range.\n",
                     elon);
            wait_for_user ();
            exit (1);
     ...
     */
    ilat = 0;
    while (betaTable[ilat + 1] < ecl_lat && ilat < nlat - 2)
        ilat++;
    ilon = 0;
    while (dlonTable[ilon + 1] < ecl_dlon && ilon < nlon - 2)
        ilon++;
    ...
    sky05 = exp (log (skyTable[ilat + (ilon)*nlat]) * (1. - frlat) * (1. - frlon)
    ...
    for (ilambda = 0; ilambda < Nlambda; ilambda++)
        {
             ...
             * Here sky05*fco*sun_spec*dlambda is the power per unit area per unit solid
             * angle in this wavelength range (Units: 1e-8 W/m^2/sr).
             *
             * The conversion from 1e-8 W --> photons/sec is 5.03411747e10*lambda(um).
             */
            zodi_tot
                    += sky05 * fco * sun_spec * dlambda * 5.03411747e10 * lambda * get_throughput (lambda, T);
        }

    /* We now have the zodi level in photons/m^2/sr/sec. Convert to
     * photons/m^2/arcsec^2/sec. */
    return (zodi_tot / 4.2545170296152206e10);
}

/* --- ROUTINES TO COMPUTE NOISE AND RELATED QUANTITIES --- */

/* Obtains the blackbody rate in units of photons per micron^2 per second per sr
 * through the filter defined by Thr in the wavelength range lambda_min ..
 * lambda_max (in um) at temperature T (in K).
 *
 * Currently uses 8192 linearly spaced steps across the waveband.
 */
double
get_BB_flux_filter (THROUGHPUT_DATA *Thr, double lambda_min, double lambda_max, double T)
{
...
/* Computes thermal emission count rate in the detector due to the telescope
 * only, as a function of: Thr                throughput table for telescope,
 * excluding filter lambda_min         minimum wavelength of filter (microns)
 ...
 *   fratio             f/ratio of the telescope (open aperture only --
 * increased by outer mask) ismask             flag: 0 = no cold mask, 1 = with
 * cold mask centobs            linear central obscuration (only used w/o mask)
 *   th_out_rad         outer radius of thermal emission annulus divided by
 * outer radius of primary mirror (>1; only used w/o mask) pmsm_emissivity
 * emissivity of PM + SM mirror surfaces
 *
 * Output is in e/pix/s, without margin.
 */
double
getThermalBkgndTel (THROUGHPUT_DATA *Thr, double lambda_min, double lambda_max, double filter_throughput,
                    double Ttel, double fratio, int ismask, double centobs, double th_out_rad,
                    double pmsm_emissivity)
{
     ...
     *
     * The reflectivities of PM/SM are divided out since some structures are seen
     * "directly" after the TM/folds, and do not have to bounce off the PM/SM on
     * their way to the detector -- so we divide by the reflection coefficients.
     */
    bbrate = filter_throughput * PIXSIZE_PHYS * PIXSIZE_PHYS
             * get_BB_flux_filter (Thr, lambda_min, lambda_max, Ttel) / (1. - pmsm_emissivity)
             / (1. - pmsm_emissivity);
     ...
     * the sky (times the effective emissivity), plus the black surface
     * contributions.
     */
...
/* Computes the total thermal background rate in the detector from all sources
 * (telescope, aft optics, and sources downstream of the filter). Output in
 * e-/pix/s.
 *
 * If component==0 returns sum of all sources; for component==1, 2, 3, returns
 * just the specified source.
 */
double
getThermalBkgndTot (THROUGHPUT_DATA *Thr, double lambda_min, double lambda_max, double filter_throughput,
                    double fratio, double centobs, THERMAL_DATA *thermal, int component)
{
    ...
    /* Telescope */
    bkgnd1 = getThermalBkgndTel (Thr, lambda_min, lambda_max, filter_throughput, thermal->T_tel, fratio,
                                 thermal->ismask, centobs, thermal->outer_ann_ratio, thermal->pmsm_emissivity)
             * THERMAL_MARGIN_FACTOR;

    /* Aft optics */
    bkgnd2 = filter_throughput * PIXSIZE_PHYS * PIXSIZE_PHYS
             * get_BB_flux_filter (Thr, lambda_min, lambda_max, thermal->T_aft) * thermal->aft_net_emissivity
             / (1. - thermal->aft_net_emissivity) / (1. - thermal->pmsm_emissivity)
             / (1. - thermal->pmsm_emissivity)
             * (thermal->ismask ? 1 : thermal->outer_ann_ratio * thermal->outer_ann_ratio) * M_PI / 4.
             / (fratio * fratio) * THERMAL_MARGIN_FACTOR;
...
double
getNoiseTotal (double t_exp, double ct, double rnfloor, int mode)
{
    ...
    switch (mode)
         ...
         * Floor is added in quadrature (mode 1) or as a replacement if the formula
         * goes lower (mode 2).
         */
        case 1:
        case 2:
        ...
        case 4:
            ...
            if (n < 2 * m)
                {
                    fprintf (stderr,
                             "Error: getNoiseTotal: Fowler-%d illegal for exposure of <%d "
                             "frames.\n",
                             (int)m, 2 * (int)m);
                    wait_for_user ();
                    exit (1);
...
void
error_check_thermal (THERMAL_DATA *thermal)
{
    /* Error checks */
    if (thermal->T_tel <= 0)
        {
    ...
    if (thermal->post_filter_rate > 0.05)
        {
            fprintf (stderr,
                     "Warning: post-filter thermal background %12.5lE e-/pix/s is "
                     "unusual.\n",
                     thermal->post_filter_rate);
        }
}
 ...
 *   T_tel pmsm_emissivity outer_ann_ratio ismask T_aft aft_net_emissivity
 * post_filter_rate Performs basic error checks.
 */
void
read_thermal_data (char Data[], THERMAL_DATA *thermal)
{
    if (sscanf (Data, "%lg %lg %lg %d %lg %lg %lg", &(thermal->T_tel), &(thermal->pmsm_emissivity),
                &(thermal->outer_ann_ratio), &(thermal->ismask), &(thermal->T_aft),
                &(thermal->aft_net_emissivity), &(thermal->post_filter_rate))
        != 7)
        {
...
main (void)
{
    ...
    printf ("Exposure time calculator v17\n");
#ifdef WL_MODE
    printf ("Mode: WL");
#endif
#ifdef SPCONT_MODE
    ...
    if (tel_config == 0)
    ...
    else
        {
             ...
             * See the manual for explicit information. The currently implemented ones
             * are: !THERMAL = specifies thermal data !WAVEFRONT = wavefront error table
             * in configuration file !IMAGING = configuration file for imaging mode
            ...
            do
                {
                    ...
                    if (memcmp (InfoLine, "!WAVEFRONT", (size_t)10) == 0)
                        {
                            is_use_wavefront = 1;
                            sscanf (InfoLine + 10, "%lg %lg %lg %lg", &(wavefront.lambdabest),
                                    &(wavefront.rmswfebest), &(wavefront.slope1), &(wavefront.slope2));
#ifndef BAO_OR_SPCONT_MODE
                            fprintf (stderr, "Error: wavelength dependent wavefront error enabled "
                                             "only in spectroscopy modes at present.\n");
                            exit (1);
#endif
                        }
#ifdef BAO_OR_SPCONT_MODE
                    if (memcmp (InfoLine, "!IMAGING", (size_t)8) == 0)
                        {
                            fprintf (stderr, "Error: This is an imaging configuration file: not "
                                             "valid in a spectroscopic mode.\n");
                            exit (1);
                        }
#endif
#ifdef WL_MODE
                    if (memcmp (InfoLine, "!SLITLESS", (size_t)9) == 0)
                        {
                            fprintf (stderr, "Error: This is a slitless spectroscopy configuration "
                                             "file: not valid in an imaging mode.\n");
                            exit (1);
                        }
#endif
            ...
            if (N > N_THROUGHPUT_MAX)
                {
                    fprintf (stderr,
                             "Error: %s: number of throughput grid points %d is too many "
                             "(max=%d).\n",
            ...
            for (i = 0; i < N; i++)
                {
                    if (fscanf (fp, "%lg %lg %lg", TAll.lambda + i, TAll.throughput + i, T1.throughput + i)
                        == EOF)
                        {
                            fprintf (stderr, "Error: Unexpected end of file: %s\n", FileName);
                            wait_for_user ();
    ...
    rmswfe_um = 0.;
            ...
            if (lambda_min <= 0.22)
                {
                    fprintf (stderr,
                             "Error: lambda_min = %12.5lE micron is outside the range of "
                             "validity of background model.\n",
                             lambda_min);
                    wait_for_user ();
                    exit (1);
                }
            ...
            if (lambda_max <= lambda_min)
                {
                    fprintf (stderr,
                             "Error: lambda_max must be greater than lambda_min: "
                             "(%12.5lE,%12.5lE) micron is illegal.\n",
            ...
            if (lambda_max > 2.5)
                {
                    fprintf (stderr,
                             "Error: lambda_max = %12.5lE micron is outside the range of "
                             "validity of background model.\n",
                             lambda_max);
                    wait_for_user ();
                    exit (1);
    ...
    if (darkcurrent < 1e-4 || darkcurrent > .1)
        {
            ...
            if (significance_cut < 3.5)
                {
                    fprintf (stderr,
                             "Error: significance cut at %12.5lE sigma is outside range of "
                             "validity of",
                             significance_cut);
            ...
            if (significance_cut < 5)
                {
                    fprintf (stderr,
                             "Warning: using galaxies with %12.5lE sigma detections not "
                             "recommended.\n",
                             significance_cut);
                }
#ifdef PZCAL_MODE
...
if (max_ellip_err < .1 || max_ellip_err > .25)
    {
        fprintf (stderr, "Warning: maximum ellipticity error %12.5lE is unusual.\n", max_ellip_err);
            ...
            /* Longitude relative to the Sun -- right now assume a Sun angle of 90
             * degrees */
#ifndef IN_DLON
            ecl_dlon = 90.;
#endif
            ...
            printf ("Tilting back from Sun: |lambda-lambda(Sun)| = %7.3lf deg, "
                    "elongation = %7.3lf deg\n",
                    ecl_dlon, epsilon);
#endif

            /* --- RECOMPUTE THROUGHPUTS INCLUDING FILTER FOR IMAGING --- */
            for (i = 0; i < TAll.N; i++)
                TAll.throughput[i] *= filterthroughput;
            for (i = 0; i < T1.N; i++)
                T1.throughput[i] *= filterthroughput;

            /* --- GENERIC ETC FUNCTIONS --- */

            /* The background is computed here. It is final for all ETC modes *except*
             * the IFU, which has to be re-computed inside the lambda loop.
             */
            ...
            /* The zodi is the dominant sky background, and the only one considered
             * here. */
            ...
            for (i = 0; i < 4; i++)
                thermalbkgnd[i] = getThermalBkgndTot (&TAll, lambda_min, lambda_max, filterthroughput, fratio,
                                                      centobs, &thermal, i);

            /* Noise variance per unit area in 1 exposure in e-^2/arcsec^2 */
            var_1exp_skyonly = tot_bkgnd * M_PI / 4. * D * D * (1. - centobs * centobs) * t_exp;
            var_1exp_bkgnd
                    = var_1exp_skyonly + (darkcurrent + thermalbkgnd[0]) * t_exp / (pixscale * pixscale);
            var_1exp = getNoiseTotal (t_exp, var_1exp_bkgnd * pixscale * pixscale, readnoise, sample_mode)
                       / (pixscale * pixscale);
            ...
            for (i = 0; i <= 10; i++)
                printf ("           |");
            printf ("\n");
            for (lambda = lambda_min; lambda < 1.0000001 * lambda_max;
                 lambda += (lambda_max - lambda_min) / (double)nstep)
                {
                    ...
                    psf.sigma = sqrt ((PIX_CD / PIXSIZE_PHYS * pixscale) * (PIX_CD / PIXSIZE_PHYS * pixscale)
                                      + jitter * jitter);
                    /* lambda/D in arcsec -- hence conversion factor of 0.648/pi from
                     * micron/meter=microradians */
                    ...
                    if (is_use_wavefront)
                        psf.rmswfe = get_rmswfe_from_fcn (lambda, &wavefront) / lambda;
                     ...
                     */
                    calib_1exp = get_throughput (lambda, &T1) * M_PI / 4. * D * D * (1. - centobs * centobs)
                                 * 5.03411747e18 * lambda * t_exp
                                 * pow (10., -0.4 * gal_ebv * Galactic_Alambda__EBV (lambda));

                    printf ("%6.4lf", lambda);
                    for (i = 0; i <= 10; i++)
                        printf (" %11.5lE", get_1sigma_flux_1exp (0.1 * i, &psf, var_1exp, calib_1exp)
                                                    * significance_cut / sqrt ((double)N_exp));
                    printf ("\n");
                }
...
printf ("  z  |lambda| EE50 | dV/dz/dA  | Flim@%4.2lf\"| n targets | dN/dz/dA  "
        "|geom mean F| siglnF |skew lnF|kurt lnF|Num ph 1exp|\n",
        R12_REF);
printf ("     |  um  |arcsec| Mpc3/deg2 |   W/m2    |  Mpc^-3   |   deg^-2  |   "
        "W/m2    |        |        |        | 1e-19 W/m2|\n");
...
for (z = DZ_OUT; z < lambda_max / LAMBDA0_HA - 1; z += DZ_OUT)
    if (z > lambda_min / LAMBDA0_HA - 1)
#endif
        {
            ...
            nbar = get_n_galaxies (z, &psf, var_1exp, calib_1exp, N_exp, significance_cut, gal_pop_model,
                                   stats)
                   * completeness;

            printf ("%5.3lf %6.4lf %6.4lf %11.5lE %11.5lE %11.5lE %11.5lE %11.5lE "
                    "%8.4lf %8.4lf %8.4lf %11.5lE\n",
                    z, lambda, get_gal_size (0.5, 0.0, &psf),
                    computeDistance (z) * computeDistance (z) / computeHubble (z) / SQDEG_PER_SR,
                    get_limflux (R12_REF, &psf, var_1exp, calib_1exp, EWD_REF, (double)N_exp,
                                 significance_cut),
            ...
            dz = (z + 0.5 * DZ_OUT > lambda_max / LAMBDA0_OIII - 1 ? lambda_max / LAMBDA0_OIII - 1
                                                                   : z + 0.5 * DZ_OUT)
                 - (z - 0.5 * DZ_OUT < lambda_min / LAMBDA0_OIII - 1 ? lambda_min / LAMBDA0_OIII - 1
                                                                     : z - 0.5 * DZ_OUT);
#elif OII_GAL
            dz = (z + 0.5 * DZ_OUT > lambda_max / LAMBDA0_OII - 1 ? lambda_max / LAMBDA0_OII - 1
                                                                  : z + 0.5 * DZ_OUT)
                 - (z - 0.5 * DZ_OUT < lambda_min / LAMBDA0_OII - 1 ? lambda_min / LAMBDA0_OII - 1
                                                                    : z - 0.5 * DZ_OUT);
#else
            dz = (z + 0.5 * DZ_OUT > lambda_max / LAMBDA0_HA - 1 ? lambda_max / LAMBDA0_HA - 1
                                                                 : z + 0.5 * DZ_OUT)
                 - (z - 0.5 * DZ_OUT < lambda_min / LAMBDA0_HA - 1 ? lambda_min / LAMBDA0_HA - 1
                                                                   : z - 0.5 * DZ_OUT);
#endif
            ...
            if (!is_an_ifu)
                ...
                {
                    printf ("|lambda|eff numb|Pixels|counts per|ABmag at|ABmag at|Zodi "
                            "bkgnd|Tot bkgnd |\n");
                    printf ("|      | of 2D  |per ln|1D pix per| S/N=1  |S/N=5per|per pixel "
                            "|per pixel |\n");
                    printf ("|  um  | pixels |lambda|exp per Jy|per pix |1e4 km/s| e^2/1exp | "
                            "e^2/1exp |\n");
                }
            ...
            for (lambda = lambda_min; lambda < 1.0000001 * lambda_max;
                 lambda += (lambda_max - lambda_min) / (double)nstep)
                {
                    ...
                    /* Pixel scale, in arcsec */
                    psf.pixscale = pixscale;
                    /* 1 sigma jitter + diffusion, in arcsec. For the IFU case, charge
                     * 'diffusion' only smears the trace in 1 of the spatial dimensions, since
                     * it is a detector effect and does not move light onto a different slice.
                     * (Of course it also smears the spectral direction but we are using
                     * 'resolution' in this mode to denote the 2-pixel width.)
                     */
                    if (is_an_ifu)
                        {
                            psf.sigma = sqrt ((PIX_CD / PIXSIZE_PHYS * pixscale)
                                                      * (PIX_CD / PIXSIZE_PHYS * pixscale) / 2.
                                              + jitter * jitter);
                        }
                    else
                        {
                            psf.sigma = sqrt ((PIX_CD / PIXSIZE_PHYS * pixscale)
                                                      * (PIX_CD / PIXSIZE_PHYS * pixscale)
                                              + jitter * jitter);
                        }
                    /* lambda/D in arcsec -- hence conversion factor of 0.648/pi from
                     * micron/meter=microradians */
                    ...
                    /* Separately calculate sensitivity for the case of [i] an IFU or [ii]
                     * slitless spectroscopy */
                    if (is_an_ifu)
                        {
                            /* IFU SPECTROSCOPY */
                            /* In this case, backgrounds are re-computed at each wavelength since
                             * they vary! */
                            ...
                            var_1exp_skyonly
                                    = zodi_bkgnd * M_PI / 4. * D * D * (1. - centobs * centobs) * t_exp;
                            /* Thermal background, e/p/s */
                            for (i = 0; i < 4; i++)
                                thermalbkgnd[i] = getThermalBkgndTot (&TAll, l1, l2, filterthroughput, fratio,
                                                                      centobs, &thermal, i);
                            /* Noise in 1 exposure per pixel */
                            var_1exp_bkgnd
                                    = var_1exp_skyonly
                                      + (darkcurrent + thermalbkgnd[0]) * t_exp / (pixscale * pixscale);
                            varperpixel_1exp = getNoiseTotal (t_exp, var_1exp_bkgnd * pixscale * pixscale,
                                                              readnoise, sample_mode);
                            /* The "final" varperpixel_1exp is e^2 per pixel in a single exposure */

                            /* Now the flux for S/N=1 per spectral pixel */
                            flux1 = sqrt (varperpixel_1exp * npix_2d) / calib_1exp
                                    / sqrt ((double)N_exp);               /* in Jy */
                            mag1 = 2.5 * log (3631. / flux1) / log (10.); /* in mag AB */
                            printf (" %6.4lf %8.4lf %6.1f %10.4lE %8.4lf %8.4lf %10.4lE %10.4lE\n", lambda,
                                    npix_2d, spDispersion / pixscale, calib_1exp, mag1,
                                    mag1 - 2.5 * log (5.) / log (10.)
                                            + 1.25 * log (spDispersion / pixscale / 29.972458) / log (10.),
                    ...
                    else
                        {
                            ...
                            flux1 = sqrt (var_1exp * pixscale * thetaeff) / calib_1exp
                                    / sqrt ((double)N_exp);               /* in Jy */
                            mag1 = 2.5 * log (3631. / flux1) / log (10.); /* in mag AB */
                            printf (" %6.4lf %8.4lf %6.1f %10.4lE %8.4lf %8.4lf %8.4lf\n", lambda, thetaeff,
                                    spDispersion / thetaeff, calib_1exp, mag1,
                                    mag1 - 2.5 * log (5.) / log (10.)
                                            + 1.25 * log (thetaeff / pixscale) / log (10.),
                                    mag1 - 2.5 * log (5.) / log (10.)
                                            + 1.25 * log (spDispersion / pixscale / 29.972458) / log (10.));
                        }
                }
            ...
            printf ("PSF encircled energy:       %8.6lf, %8.6lf, %8.6lf (at 0.1\", "
                    "0.25\", 0.5\")\n",
                    get_gal_fracE (0.1, 0, &psf), get_gal_fracE (0.25, 0, &psf),
                    get_gal_fracE (0.5, 0, &psf));
#ifdef OUT_EXTRA_PSF_PROPERTIES
#if 0
  ...
  printf("EE at 80.0/84.3/93.3: %8.6lf, %8.6lf, %8.6lf as\n", get_gal_size(0.800,0.0,&psf), get_gal_size(0.843,0.0,&psf), get_gal_size(0.933,0.0,&psf));
            ...
            printf ("5 sigma pt src threshold:   %7.3lf mag AB\n",
                    2.5 * log (calib_1exp * sqrt (N_exp) / sqrt (var_1exp * get_gal_Omeff (0., &psf)) / 5.0)
                            / log (10));
            printf ("5 sigma ext src threshold:  %7.3lf mag AB [r_eff=0.3\"]\n",
                    2.5 * log (calib_1exp * sqrt (N_exp) / sqrt (var_1exp * get_gal_Omeff (0.3, &psf)) / 5.0)
                            / log (10));
            printf ("\n");
            ...
            for (r_eff = .01; r_eff <= 1.001; r_eff *= pow (10, .05))
                if (r_eff > min_reff)
                    {
                        printf (" %7.5lf %7.4lf %7.4lf  %7.5lf   %8.5lf %7.3lf\n", r_eff,
                                get_gal_Omeff (r_eff, &psf), get_shape_penalty_factor (r_eff, &psf),
                                r_eff * r_eff / (r_eff * r_eff + ee50 * ee50),
                                get_maglim_shape (r_eff, &psf, N_exp, var_1exp, calib_1exp, max_ellip_err),
                                calib_1exp * sqrt (N_exp)
                                        * pow (10, -0.4
                                                           * get_maglim_shape (r_eff, &psf, N_exp, var_1exp,
                                                                               calib_1exp, max_ellip_err))
                                        / sqrt (var_1exp * get_gal_Omeff (r_eff, &psf)));
                    }
            ...
            for (iz = 0; iz < 20; iz++)
                {
                    dn = get_dNdA_WL_gal2 (min_reff, 100 * min_reff, 0.199999 * iz, 0.199999 * (iz + 1),
                                           GalaxyCat, lambda_min, lambda_max, &psf, N_exp, var_1exp,
                                           calib_1exp, max_ellip_err, &dneff,
#ifdef OUT_WL_CAT
                                           OutGalCat
#else
                               NULL
